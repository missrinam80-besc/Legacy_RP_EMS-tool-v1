...
let CONFIG = {};
let STAFF_ROWS = [];
const CENTRAL_REPORT_OPTIONS = { documentType: 'rapport-opname', department: 'algemeen' };
...
async function initCentralReportEnhancements() {
  ...
  try {
    await window.EMSReportCentral.init({
      storageKey: CONFIG.storageKey || config?.storageKey || 'rapport-opname',
...
function fillBasicSelects() {
  ...
  fillSelect("#admissionType", CONFIG.admissionTypeOptions || []);
}
...
function collectFormData() {
  return {
    ...
    admissionType: sanitizeText($("#admissionType")?.value),
    admissionReason: sanitizeText($("#admissionReason")?.value),
    referralSource: sanitizeText($("#referralSource")?.value),
    ...
    provisionalDiagnosis: sanitizeText($("#provisionalDiagnosis")?.value),
    clinicalObservations: sanitizeText($("#clinicalObservations")?.value),
    treatmentsStarted: sanitizeText($("#treatmentsStarted")?.value),
    medicationGiven: sanitizeText($("#medicationGiven")?.value),
    admissionPlan: sanitizeText($("#admissionPlan")?.value),
...
function composeReport(data) {
  ...
  return [
    ...
    "---",
    "",
    "__**OPNAMEGEGEVENS**__",
    `- **Locatie:** ${orDash(data.location)}`,
    `- **Datum opname:** ${formatDateDisplay(data.incidentDate)}`,
    `- **Uur opname:** ${orDash(data.incidentTime)}`,
    `- **Afdeling:** ${orDash(data.department)}`,
    `- **Kamer / ruimte:** ${orDash(data.room)}`,
    `- **Type opname:** ${orDash(data.admissionType)}`,
    `- **Reden opname:** ${orDash(data.admissionReason)}`,
    `- **Herkomst / verwijzing:** ${orDash(data.referralSource)}`,
    "",
    "---",
    "",
    "__**ARTS & TEAM**__",
    ...
    "---",
    "",
    "__**VOORLOPIGE DIAGNOSE**__",
    `- ${orDash(data.provisionalDiagnosis)}`,
    "",
    "---",
    "",
    "__**KLINISCHE OBSERVATIES**__",
    `- ${orDash(data.clinicalObservations)}`,
    "",
    "---",
    "",
    "__**OPGESTARTE ZORGEN / BEHANDELING**__",
    `- ${orDash(data.treatmentsStarted)}`,
    "",
    "---",
    "",
    "__**MEDICATIE**__",
    `- ${orDash(data.medicationGiven)}`,
    "",
    "---",
    "",
    "__**OPNAMEPLAN / VERDER BELEID**__",
    `- ${orDash(data.admissionPlan)}`,
    "",
    "---",
    "",
...
function renderMarkdownPreview(markdown) {
  ...
  lines.forEach(rawLine => {
    ...
    if (trimmed === "__**OPNAME RAPPORT**__") {
      closeParagraph();
      closeLists();
      html.push(`<div class="report-title">OPNAME RAPPORT</div>`);
      return;
    }
...
function getStorageKey() {
  return String(CONFIG.storageKey || "ems-admission-report-v1");
}
...
function restoreDraft() {
  ...
  setValue("#admissionType", fields.admissionType);
  setValue("#admissionReason", fields.admissionReason);
  setValue("#referralSource", fields.referralSource);
  ...
  setValue("#provisionalDiagnosis", fields.provisionalDiagnosis);
  setValue("#clinicalObservations", fields.clinicalObservations);
  setValue("#treatmentsStarted", fields.treatmentsStarted);
  setValue("#medicationGiven", fields.medicationGiven);
  setValue("#admissionPlan", fields.admissionPlan);
  ...
  if (CONFIG.discordWebhookProxyUrl && CONFIG.discordWebhookProxyUrl.trim()) {
    ...
    try {
      ...
      await window.DiscordWebhookService.sendFormMessage({
        ...
        formType: "opname",
        content: report,
        username: CONFIG.discordUsername || "EMS Opname Tool",
        extraData: buildAdmissionDiscordMeta(),
...
function buildAdmissionDiscordMeta() {
  const data = collectFormData();
